...
template<int N>
static inline __m256 horner_desc_ps(__m256 x,
...
static inline __m256 rational_desc_ps(__m256 x,
...
static __m256 exp_erfc_tail(const __m256 x) noexcept
{
...
static __m256 erfc_tail_eval(__m256 ax, __m256 z, __m256 is_neg,
    bool tail1) noexcept
{
...
static __m256 erfc_small_branch(__m256 z, __m256 ax, __m256 neg_mask) noexcept
{
    ...
    return _mm256_blendv_ps(erfc_pos, erfc_neg, neg_mask);
}

static __m256 erfc_mid_branch(__m256 ax, __m256 neg_mask) noexcept
{
    ...
    __m256 s = _mm256_sub_ps(ax, _mm256_set1_ps(1.0f));
...
__m256 fy::simd::intrinsic::erfc(__m256 input) noexcept
{
    ...
    {
        __m256 small_res = erfc_small_branch(z, ax, is_neg);
        result = _mm256_blendv_ps(result, small_res, mask_small);
    }

    if (_mm256_movemask_ps(mask_mid))
    {
        __m256 mid_res = erfc_mid_branch(ax, is_neg);
        result = _mm256_blendv_ps(result, mid_res, mask_mid);
    }

    if (_mm256_movemask_ps(mask_tail1))
    {
        __m256 tail1_res = erfc_tail_eval(ax, z, is_neg, true);
        result = _mm256_blendv_ps(result, tail1_res, mask_tail1);
    }

    if (_mm256_movemask_ps(mask_tail2))
    {
        __m256 tail2_res = erfc_tail_eval(ax, z, is_neg, false);
        result = _mm256_blendv_ps(result, tail2_res, mask_tail2);
    }
