	...
	__m256i* out_k) noexcept
{
	...
	__m256 idx_hi_mask = _mm256_castsi256_ps(_mm256_cmpgt_epi32(idx, _mm256_set1_epi32(7)));
...
static __m256 exp2_finalize_special(__m256 result, __m256 input) noexcept
{
	...
	const __m256 abs_limit_tiny = _mm256_set1_ps(0x1p-25f);
	const __m256 overflow_limit = _mm256_set1_ps(0x1.0p7f);
	const __m256 underflow_limit = _mm256_set1_ps(-0x1.2cp7f);

	__m256 ax = _mm256_andnot_ps(sign_mask, input);
	__m256 tiny_mask = _mm256_cmp_ps(ax, abs_limit_tiny, _CMP_LE_OQ);
	...
	__m256 neginf_mask = _mm256_cmp_ps(input, 
		_mm256_set1_ps(-std::numeric_limits<float>::infinity()), _CMP_EQ_OQ);
	...
	__m256 tbl_hi) noexcept
{
	...
	__m256 result = _mm256_mul_ps(r, extra_hi);
