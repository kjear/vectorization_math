...
static __m256 ln_posfinite(const __m256 x_in) noexcept
{
	const __m256 vabs_mask = _mm256_castsi256_ps(_mm256_set1_epi32(0x7fffffff));
	const __m256 one = _mm256_set1_ps(1.0f);
	const __m256 half = _mm256_set1_ps(0.5f);
	...
	__m256 t = _mm256_sub_ps(x_in, one);
	__m256 at = _mm256_and_ps(t, vabs_mask);
	...
	__m256 y_near1;
	{
		...
		y_near1 = _mm256_fmadd_ps(x2, p, t);
	}

	__m256 x = _mm256_max_ps(x_in, min_norm);
	...
	return _mm256_blendv_ps(y, y_near1, mask_near1);
}
...
{
	const __m256i thresh_2_i = _mm256_set1_epi32(0x40000000);
...
__m256 fy::simd::intrinsic::atanh(__m256 x) noexcept
{
	...
	__m256 t_abs = _mm256_mul_ps(vhalf, log1p_nonneg_atanh_dispatch(arg));
